...
kubectl config --kubeconfig /vagrant/kubernetes/kubeconfig set-cluster local --server https://master.example.com --certificate-authority /etc/kubernetes/ssl/ca.pem
