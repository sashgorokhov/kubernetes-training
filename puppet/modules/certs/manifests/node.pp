...
class certs::node inherits certs::params {
  ...
  exec {'node.pem':
    command => "/usr/bin/openssl x509 -req -in ${ssl_dir}/node.csr -CA ${ssl_dir}/ca.pem -CAkey ${ssl_dir}/ca-key.pem -CAcreateserial -out ${ssl_dir}/node.pem -days 365 -extensions v3_req -extfile ${ssl_dir}/node.cnf",
