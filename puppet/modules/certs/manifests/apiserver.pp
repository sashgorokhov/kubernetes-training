...
class certs::apiserver inherits certs::params{
  ...
  exec {'apiserver.pem':
    command => "/usr/bin/openssl x509 -req -in ${ssl_dir}/apiserver.csr -CA ${ssl_dir}/ca.pem -CAkey ${ssl_dir}/ca-key.pem -CAcreateserial -out ${ssl_dir}/apiserver.pem -days 365 -extensions v3_req -extfile ${ssl_dir}/apiserver.cnf",
